...
allocate_block(void)
{
	...
	{
		if (bitvector_test(freeBitmap, i))
		{
			bitvector_clear(freeBitmap, i);
			return i;
		}
	}
	return 0;
...
free_block(uint32_t blockno)
{
	/* EXERCISE: Your code here */
	void *freeBitmap = ospfs_block(OSPFS_FREEMAP_BLK);
	if (blockno >= OSPFS_FREEMAP_BLK && blockno < ospfs_super -> os_nblocks)
	{
...
direct_index(uint32_t b)
{
	...
	return ((b - OSPFS_NDIRECT) % OSPFS_NINDIRECT);  //TACO check
}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if (copy_to_user(buffer, data + offset, n)) //copy_to_user will return number of bytes which were not copied, on success will be zero
		{
			return -EFAULT;
		}
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if (copy_from_user(data + (*f_pos % OSPFS_BLKSIZE), buffer, n)) //copy_from_user will return number of bytes which were not copied, on success will be zero
		{
			return -EFAULT;
		}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	// Search through directory block for an empty entry 
	int entry_off; 
	ospfs_direntry_t *od; 
	for(entry_off = 0; entry_off < dir_oi->oi_size; entry_off += OSPFS_DIRENTRY_SIZE)
	{
		// Retrieve an entry and check if it is empty
		od = ospfs_inode_data(dir_oi, entry_off);
		if(od->od_ino == 0) 
		{
			if(IS_ERR(od)) // Check if od is an error value
				return PTR_ERR(od);
			else
				return od; 
		}
	}
	// Else, we must add a block to the directory 
	uint32_t new_size = (ospfs_size2nblocks(dir_oi->oi_size)+1)*OSPFS_BLKSIZE;
	switch(change_size(dir_oi, new_size))
	{
		case 0: // change_size succeeds
			dir_oi->oi_size = new_size; // Update stored size
			od = ospfs_inode_data(dir_oi, entry_off); // Retrieve entry at newly allocated block 
			return od;
